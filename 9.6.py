...
if r1 + r2 > abs(x1 - x2) and r1 + r2 > abs(y1 - y2):
    if abs(y1 - y2) == math.sqrt((r1 + r2)**2 - (x2 - x1)**2) or abs(x1 - x2) == math.sqrt((r1 + r2)**2 - (y2 - y1)**2):
        plt.title('Окружности имеют внешнее касание', fontsize=8, color='red')
    elif abs(y1 - y2) > math.sqrt((r1 + r2)**2 - (x2 - x1)**2) or abs(x1 - x2) > math.sqrt((r1 + r2)**2 - (y2 - y1)**2):
